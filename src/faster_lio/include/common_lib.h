...
#include "faster_lio_interfaces/msg/pose6_d.hpp"
#include "options.h"
#include "so3_math.h"
...
inline std::string DEBUG_FILE_DIR(const std::string &name) { return "Log/" + name; }

using Pose6D = faster_lio_interfaces::msg::Pose6D;
...
inline double GetSecondsFromHeader(const std_msgs::msg::Header& header) {
    return static_cast<double>(header.stamp.sec) +
           static_cast<double>(header.stamp.nanosec) * 1e-9;
}

inline builtin_interfaces::msg::Time SecondsToTimestamp(double seconds) {
    builtin_interfaces::msg::Time time;
    time.sec = static_cast<int32_t>(seconds);
    time.nanosec = static_cast<int32_t>((seconds - time.sec) * 1e9);
    return time;
}
